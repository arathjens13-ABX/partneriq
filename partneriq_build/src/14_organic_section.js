...
function detectFileType(filename, rows = []) {
  ...
  // Tokenize the ORIGINAL-case filename on both non-alphanumeric separators and
  // CamelCase boundaries, so "TVVisibleSignage" → ["tv","visible","signage"],
  // "TV_signage" → ["tv","signage"], and "shortvideo" stays ["shortvideo"].
  // Matching whole tokens (not substrings) is what keeps "shortvideo" from
  // reading as TV, while the CamelCase split is what makes separator-free names
  // like the real "TVVisibleSignage" export resolve correctly.
  const _fileWords = String(filename || '')
    .replace(/\.(csv|xlsx|xls)$/i, '')
    .replace(/([a-z0-9])([A-Z])/g, '$1 $2')     // camelCase:  tvVisible → tv Visible
    .replace(/([A-Z]+)([A-Z][a-z])/g, '$1 $2')  // ACRONYMWord: TVVisible → TV Visible
    .split(/[^a-zA-Z0-9]+/)
    .filter(Boolean)
    .map(w => w.toLowerCase());
  ...
  // TV Visible Signage by column schema — distinctive enough (Brand + Matchdate
  // + a QI/Tool column) that TV data survives even if the filename convention
  // changes. TV has no partner-neutral overlap with the Nielsen ratings schema,
  // which is matched above and needs 'hh rtg' + 'demo' + 'opponent'.
  if (hasCol('brand') && (hasCol('matchdate') || hasCol('match date')) &&
      (hasCol('tool') || hasCol('qi media value ($)') || hasCol('sponsorship qi impressions'))) {
    return 'tv';
  }
